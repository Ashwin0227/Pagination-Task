...
var result = [
  ...
  function child(e){
    ...
    var returned = parseInt(e.getAttribute('for'))
    var obtainedData = result[returned-1]
    var creatingDiv = document.createElement('div')
    creatingDiv.innerHTML =`<p for="${obtainedData}">Id : ${obtainedData.id}</p>
   <p for="name">Name : ${obtainedData.name}</p>
   <p for="email">Email : ${obtainedData.email}</p>`
   document.querySelector('.parent-data').append(creatingDiv).classList.add('.divison')
   }
  ...
  btnNext.addEventListener('click',()=>{
    for(var i = 1; i < 11; i++){
      var attr = document.querySelector(`#btn-${i}`)
      var getAttr =parseInt(attr.getAttribute('for'))
      var newAttr = getAttr + 10;
      if(result.length >= newAttr){
        attr.setAttribute('for', newAttr)
        attr.innerHTML = newAttr
      }
